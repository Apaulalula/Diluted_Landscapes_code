...
function simulate_coevolution(f, z, phi_c, phi_r, alpha, m_r, m_c, epsilon, theta, z_r, z_c, r_int_sp, c_int_sp, i, j)
    ...
    m = vcat(repeat([m_r], n_r_current), repeat([m_c], n_c_current))
     ...
     # vector of phi values for all species
     phi = vcat(repeat([phi_r], n_r_current), repeat([phi_c], n_c_current))
...
function simulate_coevolution_or_evolution(z_r, z_c, x_c, r_current_sp, c_current_sp, theta_r, theta_c, alpha, m_r, m_c, epsilon, phi_c, phi_r, M_inc, i, j)
    ...
    if sum(current_inc) != 0
        ...
        z_r, z_c = simulate_coevolution(f, z, phi_c, phi_r, alpha, m_r, m_c, epsilon, theta, z_r, z_c, r_int_sp, c_int_sp, i, j)
    ...
    if length(r_nonint_sp) != 0
        ...
        z_r = simulate_evolution(r_nonint_sp, z_r, phi_r, theta_r, i, j)
        z_c = simulate_evolution(c_nonint_sp, z_c, phi_c, theta_c, i, j)
